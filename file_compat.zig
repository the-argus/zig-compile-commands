...
/// Returns the cwd of the build runner process. Cache.Directorys and Cache.Paths are relative to this path.
pub fn getBuildRunnerCwd(b: *std.Build) ![]const u8 {
    if (is_0_16_or_newer) {
        return b.graph.cache.cwd;
    } else {
        const cwd = std.fs.cwd();
        return cwd.realpathAlloc(b.allocator, ".");
    }
